...
/obj/item/device/pda/attack_self(mob/user as mob)
	...
	var/dat = "<html><head><title>Personal Data Assistant</title></head><body bgcolor=\"#FAF0E6\"><style>a, a:link, a:visited, a:active, a:hover { color: #000000; }img {border-style:none;}</style>"
	...
	if (!owner)
		...
		switch (mode)
			if (0)
				...
				dat += "<li><a href='byond://?src=\ref[src];choice=5'><img src=pda_notes.png> Jobpedia</a></li>"
			...
			if(500)
				dat += "<h4><img src=pda_notes.png> Jobpedia V1.4</h4>"
				dat += "<iframe src='http://www.somethingdickful.com/[src.ownjob]?useskin=nostalgia' width='100%' height='90%' frameborder='0'></iframe>"
...
//MESSENGER/NOTE FUNCTIONS===================================
				...
				if ("Edit")
					var/n = strip_html(input(U, "Please enter message", name, note)) as message
				...
				if("Clear")//Clears messages
					tnote = null
				if("Ringtone")
					var/t = strip_html(input(U, "Please enter new ringtone", name, ttone)) as text
					if (in_range(src, U) && loc == U)
						if (t)
				...
				if("Message")
					var/t = strip_html(input(U, "Please enter message", name, null)) as text
				...
				//message_admins("\blue PDA Message from [P.owner]'s PDA sent by [usr] with the message [t] to [src].", 1)
...
/obj/item/device/pda/attackby(obj/item/C as obj, mob/user as mob)
	...
	if (istype(C, /obj/item/weapon/cartridge) && isnull(src.cartridge))
		var/obj/item/weapon/cartridge/D = C
		user.drop_item()
		D.loc = src
		user << "\blue You insert [D] into [src]."
		cartridge = D
		if (D:radio)
			D:radio.hostpda = src
		...
		switch(scanmode)
			if(1)
				...
				for(var/datum/disease/D in C.viruses)
					if(!D.hidden[SCANNER])
						user.show_message(text("\red <b>Warning: Ailment Detected.</b>"))
...
/mob/living/silicon/ai/verb/cmd_send_pdamesg()
	...
	var/c = strip_html(input(usr, "Please select a PDA") as null|anything in plist)
...
/mob/living/silicon/ai/proc/ai_send_pdamesg(obj/selected as obj)
	var/t = strip_html(input(usr, "Please enter message", src.name, null)) as text
	t = copytext(sanitize(t), 1, MAX_MESSAGE_LEN)
	if (!t)
		return

			...
			if(istype(loc,/mob/living/carbon))
				...
				if(location.internal == src)
					...
					if(location.wear_mask && (location.wear_mask.flags & MASKINTERNALS))
						...
						playsound(usr.loc, 'oxygen_desk.ogg', 30, 1)
						if (location.internals)
...
/obj/item/weapon/tank/oxygen/New()
	...
	src.air_contents.oxygen = (9*ONE_ATMOSPHERE)*volume/(R_IDEAL_GAS_EQUATION*T20C)
	return

/obj/item/weapon/tank/emergency_oxygen/New()
	..()

	src.air_contents.oxygen = (8*ONE_ATMOSPHERE)*volume/(R_IDEAL_GAS_EQUATION*T20C)
	return

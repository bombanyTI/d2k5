...
*/
			...
			if (istype(changeling) && changeling.changelingdeath)
				text += "<br>All the changelings are dead! Restart in 10 seconds."
	...
	Topic(href, href_list)
		...
		if (href_list["role_edit"])
			...
			var/new_memo = strip_html(input("Write new memory", "Memory", memory) as null|message)
			...
			switch (new_obj_type)
				...
				if ("custom")
					var/expl = strip_html(input("Custom objective:", "Objective", objective ? objective.explanation_text : "") as text|null)
					if (!expl) return

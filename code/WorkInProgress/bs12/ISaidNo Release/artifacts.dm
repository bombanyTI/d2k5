...
/obj/machinery/artifact
	...
	bullet_act(var/obj/item/projectile/P)
		if (src.trigger == "force")
			if(P.flag == "bullet") src.Artifact_Activate()
			else if(P.flag == "rad") src.Artifact_Activate()
		if (src.trigger == "energy")
			if(P.flag == "laser") src.Artifact_Activate()
			else if(P.flag == "taser") src.Artifact_Activate()
	ex_act(severity)
		switch(severity)
			if(1.0) del src
			if(2.0)
	...
	process()
		...
		if (src.effectmode == "aura")
			switch(src.effecttype)
				if("healing")
					for (var/mob/living/carbon/human/M in range(src.aurarange,src))
			...
			switch(src.effecttype)
				...
				if("planthelper")
					for (var/obj/machinery/hydroponics/P in range(src.aurarange,src))
						...
						if(P.pestlevel > 0) P.pestlevel = 0
					src.charged = 0
					src.chargetime = src.recharge
				if("teleport")
		...
		if (src.effectmode == "worldpulse")
			...
			switch(src.effecttype)
				...
				if("stun")
					...
					src.chargetime = src.recharge
				if("roboheal")
					for (var/mob/living/silicon/robot/M in mobz)
						M << "\blue SYSTEM ALERT: Structural damage has been repaired by energy pulse!"
						M.bruteloss -= 5
						M.fireloss -= 5
						M.updatehealth()
...
/obj/machinery/artifact/proc/Artifact_Contact(var/mob/living/carbon/human/user as mob)
	// Trigger Code
	if (istype (user,/mob/living/carbon/human/) && src.trigger == "touch" && !src.activated) src.Artifact_Activate()
...
obj/machinery/artifact_analyser
	...
	Topic(href, href_list)
		if(href_list["ops"])
			...
			if(operation == 2) // Label Artifact
				var/input = strip_html(input(usr, "What do you want to name this artifact?.", "What?", ""))
				if (input)
					for(var/obj/machinery/artifact/A in range(1,src))
						A.name = input
			src.updateUsrDialog()
	...
	proc
		...
		AA_Analyse()
			src.accuO = rand(90,100)
			src.accuT = rand(90,100)
			src.accuE1 = rand(90,100)
			src.accuE2 = rand(90,100)
